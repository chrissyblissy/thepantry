...
db = SQLAlchemy(app)
engine = create_engine('mysql://updatepantry:K269210dL1ghtn1ng@localhost/thepantry')
...
def insertifnot(columnvalue, value):
    ...
    for item in db.engine.execute("SELECT * FROM " + columnvalue + "s"):
        if value.lower() == item[1].lower():
            ...
            return item[0]
    if itemexists == False:
        sqlsession.execute(insert(Table(columnvalue + 's', metadata, autoload=True)).values({columnvalue +"_name": value}))
        lastid = sqlsession.execute("SELECT LAST_INSERT_ID() AS " + columnvalue + "id")
        for row in lastid:
...
def myrecipes():
    ...
    userrecipesinput = "(SELECT recipe_id FROM users_recipes WHERE user_id=" + str(session.get("user_id")) + ")"
...
@login_required
def mypantry():
    useringredientsinput = "(SELECT ingredient_id FROM users_ingredients WHERE user_id=" + str(session.get("user_id")) + ")"
    sqlinput = "SELECT ingredient_name, ingredient_type FROM ingredients WHERE ingredient_id IN " + useringredientsinput + " ORDER BY ingredient_type, ingredient_name ASC"
...
def allingredients():
    ...
    if request.method =="POST":
        ...
        fooddata = db.engine.execute("SELECT * FROM ingredients ORDER BY ingredient_type, ingredient_name ASC")
